...
type DayBlock = { title: string; videoUrl: string; mantra?: string; description?: string };
...
export type EditProgramFormShape = {
    slug: string;
    title: string;
...
function normalizeUrl(u?: string) {
    ...
    if (/^(youtu\.be|((www\.)?youtube\.com)|vimeo\.com)\//i.test(s)) return `https://${s}`;
    return s;
}
...
export default function EditProgramForm({ initialData }: { initialData: EditProgramFormShape }) {
    ...
    const onSubmit: SubmitHandler<EditProgramFormShape> = async (values) => {
        try {
            setSaving(true);
            setErr(null);
            ...
            <div className="sticky top-0 z-30 -mx-1 bg-white/95 px-1 py-3 backdrop-blur">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="min-w-0">
                        <div className="text-xs text-gray-500">Éditer le programme</div>
                ...
                <Card>
                    ...
                    <div className="grid md:grid-cols-3 gap-3">
                        <div>
                            ...
                            <InputPill {...register('slug', { required: true })} readOnly />
                            <Help>Le slug est la clé du programme.</Help>
                        </div>
                        <div>
                            <Label required>Titre</Label>
                            <InputPill {...register('title', { required: true })} placeholder="RESET-7" />
                        </div>
                        <div>
                            <Label>Niveau</Label>
