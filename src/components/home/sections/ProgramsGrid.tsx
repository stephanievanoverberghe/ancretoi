...
export default async function ProgramsGrid() {
    ...
    const pages = (await ProgramPage.find({}).select({ programSlug: 1, status: 1, hero: 1, meta: 1, price: 1 }).sort({ createdAt: 1 }).lean()) as PgLean[];
    ...
    const programs: Program[] = pages.map((p) => ({
        ...
        /* ✅ Prix côté client */
        price: p.price?.amountCents != null ? { amount_cents: p.price.amountCents!, currency: (p.price.currency ?? 'EUR') as string } : null,
    }));
